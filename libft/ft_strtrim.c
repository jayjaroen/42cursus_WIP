...
/*   Updated: 2023/09/26 22:55:19 by jay              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"
/*
Descritption: Allocates (with malloc(3)) and returns a copy of
...
char  *ft_strtrim(char const *s1, char const *set)
{
  ...
  trim = (char *)malloc(sizeof(char) * (end - start + 2)); //index + null terminator
  if (trim == NULL)
    return (NULL);
  ft_strlcpy(trim, s1 + start, end - start + 2); // does it work with blank input / tab
  return (trim);
}
